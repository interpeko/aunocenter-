...
const WINDOW_MS=60_000;const MAX_REQUESTS=20;
export async function handler(event){
  if(event.httpMethod!=='GET') return json(405,{error:'Method not allowed'});
  const now=Date.now();const client=event.headers?.['x-nf-client-connection-ip']||event.headers?.['x-forwarded-for']?.split(',')[0]?.trim()||'unknown';
  const window=requestWindows.get(client);if(window&&now-window.start<WINDOW_MS&&window.count>=MAX_REQUESTS)return json(429,{error:'Too many searches. Please wait a minute and try again.'});
  if(!window||now-window.start>=WINDOW_MS)requestWindows.set(client,{start:now,count:1});else window.count+=1;
  if(requestWindows.size>500)for(const [key,value] of requestWindows)if(now-value.start>=WINDOW_MS)requestWindows.delete(key);
  ...
  try{
    ...
    const searchResponse=await fetch(searchUrl,{headers:{Accept:'application/json','User-Agent':'AUNO-Center-PubMed-Interface/1.0'},signal:AbortSignal.timeout(8_000)});
    ...
    const summaryResponse=await fetch(summaryUrl,{headers:{Accept:'application/json','User-Agent':'AUNO-Center-PubMed-Interface/1.0'},signal:AbortSignal.timeout(8_000)});
